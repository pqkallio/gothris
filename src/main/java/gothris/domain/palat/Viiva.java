...
public class Viiva extends Kompositio {
    
    public Viiva(int x, int y, int palanKoko) {
        super(x, y, palanKoko, new Color(102, 51, 0));
        List<Pala> palaset = new ArrayList<Pala>();
        Collections.addAll(palaset, new Pala(x, y), new Pala(x, y+1),
                new Pala(x, y+2), new Pala(x, y+3));
    ...
    public void kaanna() {
        if ( this.rotaatio == 1 ) {
            this.osat.get(0).setX(this.osat.get(0).getX()-2);
            this.osat.get(0).setY(this.osat.get(0).getY()+3);
            
            this.osat.get(1).setX(this.osat.get(1).getX()-1);
            this.osat.get(1).setY(this.osat.get(1).getY()+2);
